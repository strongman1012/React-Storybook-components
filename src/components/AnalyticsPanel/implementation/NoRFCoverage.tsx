...
export interface NoRFCoverageProps {
    dataset: any
...
const NoRFCoverage: FC<NoRFCoverageProps> = ({
    dataset,
    selectedUser,
    updateSelectedUser
}) => {
    ...
    useEffect(() => {
        ...
        if(uniqueInclinations.length > 1){
            setIs3D(true);
            setPlotData([{
                x: dataset.modelData.orbital['averageGapDuration']?.points?.map((x: { altitude: number; }) => x.altitude) ?? [selectedUser.altitude],
                y: dataset.modelData.orbital['averageGapDuration']?.points?.map((x: { inclination: number; }) => x.inclination) ?? [0],
                z: dataset.modelData.orbital['averageGapDuration']?.points?.map((x: { value: number; }) => x.value) ?? [0],
                mode: 'markers',
                type: 'scatter3d',
                opacity: 1,
                marker: {
            ...
            setPlotData([{
                x: dataset.modelData.orbital['averageGapDuration']?.points?.map((x: { altitude: number; }) => x.altitude) ?? [selectedUser.altitude],
                y: dataset.modelData.orbital['averageGapDuration']?.points?.map((x: { value: number; }) => x.value) ?? [0],
                name: '',
                type: 'scatter',
                mode: 'lines+markers',
                marker: { color: 'red' }
                                ...
                                values: [
                                    {
                                        ...
                                        config: { displayModeBar: false },
                                        graphData: [{
                                            x: traces ? traces['gap']?.xTraces : null,
                                            y: traces ? traces['gap']?.yTraces : null,
                            ...
                            values: [
                                {
                                    ...
                                    graphData: [
                                        {
                                            y: traces ? traces['gap_histogram']?.xTraces : null,
                                            boxpoints: 'all',
                                            name: '',
                                            type: 'box'
                                        }
