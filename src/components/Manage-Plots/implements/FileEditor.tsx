...
export interface FileEditorProps extends CodeEditorProps {
    ...
    saveTrigger: boolean,
    setSaveTrigger: (trigger: boolean) => void,
...
const FileEditor: FC<FileEditorProps> = ({
    ...
    useEffect(() => {
        if (currentFile !== undefined) {
            ...
            if (currentTab === undefined) {
                const newTabs = [...tabs, { id: currentFile.id, name: currentFile.name, ext: currentFile.ext }];
                setTabs(newTabs);
    ...
    const handleCodeChange = (evn: any) => {
        ...
        if (currentFile !== undefined) {
            ...
            // Update the codes array with the new code details
            setCodes([...codes.filter((x) => x.id !== currentFile.id), updatedCode]);
            setCodeChanged(true);

            // Update the currentFile object with the new script
            updateCurrentFile({ ...currentFile, script: newCode });
        }
    }
